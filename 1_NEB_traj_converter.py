...
def manual_parse_and_save(input_filename='espresso.neb.in'):
    ...
    try:
        with open(input_filename, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if '&SYSTEM' in line_upper:
                    ...
                    continue
                if in_system_section and '/' in line_upper:
                    in_system_section = False
                    continue
                ...
                if 'ATOMIC_POSITIONS' in line_upper:
                    if current_image_key: # FIRST 또는 LAST 이미지 섹션일 때만 좌표 읽기 시작
                ...
                if in_system_section:
                    ...
                    if len(parts) >= 2 and parts[0].lower() in ['a', 'b', 'c', 'cosab']:
                        try:
                            cell_params[parts[0].lower()] = float(parts[1])
                        except ValueError:
                            pass # 값 변환 실패 시 무시
                ...
                if is_reading_coords and current_image_key:
                    ...
                    if len(parts) >= 4 and parts[0].isalpha():
                        ...
                        images_coords[current_image_key].append({'symbol': symbol, 'position': position})
        ...
        if not all([a, b, c, cosab is not None]):
            print(f"오류: 셀 파라미터(a, b, c, cosab)를 '{input_filename}'의 &SYSTEM 블록에서 찾을 수 없습니다.")
            sys.exit(1)
        ...
        if not initial_data or not final_data:
            print("오류: FIRST_IMAGE 또는 LAST_IMAGE의 좌표를 찾을 수 없습니다.")
            sys.exit(1)

        initial_atoms = Atoms(symbols=[d['symbol'] for d in initial_data],
                              positions=[d['position'] for d in initial_data],
                              cell=cell, pbc=True)
        write('initial_unoptimized.traj', initial_atoms)
        print("-> 'initial_unoptimized.traj' 파일이 생성되었습니다.")
        
        final_atoms = Atoms(symbols=[d['symbol'] for d in final_data],
                            positions=[d['position'] for d in final_data],
                            cell=cell, pbc=True)
